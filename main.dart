...
class OrderItem {
  ...
  String toKitchenLine() {
    ...
    final note = chefNote.trim().isEmpty ? '' : ' — NOTA: $chefNote';
    return '${qty}× ${product.name}$mods$note';
  }
            ...
            child: active.isEmpty
              ...
              itemBuilder: (context, i) {
                ...
                return ListTile(
                  ...
                  title: Text('${t.id} • ${t.isTakeaway ? 'Asporto' : 'Tavolo'}'),
                  subtitle: Text(t.isTakeaway
                      ? (t.pickupTime != null ? 'Ritiro: ${t.pickupTime!.format(context)}' : 'Ritiro: —')
                ...
                children: [
                  ...
                  if (takeaway)
                    ListTile(
                      ...
                      title: Text('Ora ritiro: ${pickup == null ? '—' : pickup!.format(ctx)}'),
...
class _TableDetailPageState extends State<TableDetailPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('${table.id} • ${table.isTakeaway ? 'Asporto' : 'Tavolo'}'),
        actions: [
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 16.0, vertical: 8),
            child: Center(child: Text('Totale: € ${table.total.toStringAsFixed(2)}')),
          ),
        ],
      ),
      body: Column(
...
class ReceiptDetailPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text('Scontrino • ${table.id}')),
      body: Padding(
        padding: const EdgeInsets.all(12.0),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.stretch,
                ...
                children: [
                  ...
                  if (state.modifiers.isNotEmpty) ...[
                    ...
                    ...state.modifiers.map((mod) => CheckboxListTile(
                      title: Text('${mod.name} (+€${mod.price.toStringAsFixed(2)})'),
        ...
        children: [
          ...
          if (filteredTables.isEmpty)
            ...
            _ReportCard(
              ...
              value: '€ ${dineInRevenue.toStringAsFixed(2)}',
              icon: Icons.table_restaurant,
            ),
            const SizedBox(height: 8),
            _ReportCard(
              title: 'Incasso Asporto',
              value: '€ ${takeawayRevenue.toStringAsFixed(2)}',
              icon: Icons.fastfood,
            ),
            ...
            const Divider(height: 32),
            const Text('Prodotti più Venduti', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
            const SizedBox(height: 16),
            ListView.builder(
              shrinkWrap: true,
    ...
    showDialog(
      ...
      builder: (BuildContext dialogContext) {
        return AlertDialog(
          ...
          content: const Text('Sei sicuro di voler eliminare tutti i dati? Questa azione è irreversibile.'),
          actions: [
            TextButton(
              onPressed: () => Navigator.of(dialogContext).pop(),
              child: const Text('Annulla'),
            ...
            children: [
              ...
              if (item.selectedModifierIds.isNotEmpty)
                ...item.selectedModifierIds.map((modId) {
                  ...
                  return Text(
                    '  - ${mod.name} (+€${mod.price.toStringAsFixed(2)})',
